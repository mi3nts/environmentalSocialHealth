...
def test_train_plot(full_pdf, y_test, train_r2, y_train, X_train, test_r2, X_test, title, save_path=None):
...
def plot_qq(full_pdf, save_path):
    ...
    # plt.savefig(f"Plots/DUEDARE/{str(ybear)}/{plot_path}/PM Size/{name}/{name}_qqplot.png")
    # plt.show()
    #plt.savefig(save_path)
    plt.show()
